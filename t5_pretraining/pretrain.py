...
#         rouge_dict = self.parse_score(rouge_results)    
    ...
    def training_step(self, batch, batch_idx):
        ...
        res = {"loss": loss, "log": tensorboard_logs}
    ...
    def validation_step(self, batch, batch_idx):
        pred = self._generative_step(batch)
    ...
    def train_dataloader(self):   
        ...
        dataloader = DataLoader(train_dataset, batch_size=self.hparams.train_batch_size, drop_last=True, num_workers=0)
        t_total = (
...
prefix_path='./t5_pretraining/data/' #Path to custom training data. Name the training corpus train_context.csv
class Pretrain(Dataset):
    def __init__(self, tokenizer, type_path, num_samples, input_length, output_length, print_text=False):
        df = pd.read_csv(prefix_path+"train_context.csv")
        if(num_samples > 0):
            df = df.sample(num_samples)
        self.dataset = self.split_into_segment(df,input_length)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='t5_pretraining/t5_pretrained')
    parser.add_argument('--train_batch_size', default=8)
    parser.add_argument('--learning_rate', default=1e-3)
    parser.add_argument('--model', default='t5_pretraining/t5-base')
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        dirpath=args.output_dir, filename="checkpoint")
